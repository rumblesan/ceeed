...
CodeMirror.defineMode('ceed', function () {
  ...
  const sequenceRe = tokenIdentifiers.sequence.regexp;

  const arrowRe = /^[-=>]>/;

  const globalBusses = ['time', 'out'];
  ...
  function tokenise(stream, state) {
    ...
    if ((match = stream.match(identifierRe))) {
      ...
      return 'comment';
    } else if (stream.match(sequenceRe)) {
      return 'variable';
    }
...
CodeMirror.defineMIME('text/x-ceed', 'ceed');
