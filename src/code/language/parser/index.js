...
parser.baseSource = function () {
  ...
  if (this.la1('number')) {
    ...
    return source;
  } else if (this.la1('sequence')) {
    return this.sequence();
  }
};

parser.sequence = function () {
  const seqPosition = this.position();
  const seq = this.match('sequence').content;
  ...
  for (let i = 0; i < seq.length; i += 1) {
    ...
    if (current === '') {
      ...
      values.push(parseInt(current, 10));
  ...
  if (current != '') {
    ...
    // spaces to the final values duration
    durations.push(spaces + offset);
  }
  return ast.Sequence(values, durations, offset, seqPosition);
};
